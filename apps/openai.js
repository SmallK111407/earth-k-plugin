...
export class example extends plugin {
    ...
    async ly(e) {
        ...
        let url = 'https://api.caonm.net/api/ai/ya?key=JMHz65uMHMZ3WCpl4jQ1U1blRr&length=1.1&speaker='+js+'&msg='+kw
    ...
    //https://api.caonm.net/api/ai/ya?key=JMHz65uMHMZ3WCpl4jQ1U1blRr&msg=%E4%BD%A0%E5%A5%BD
    async xx(e) {
        let kw = e.msg.replace(/老星/, "").trim()
        let url = 'https://api.caonm.net/api/ai/hx?key=JMHz65uMHMZ3WCpl4jQ1U1blRr&message='+kw
        let res = await fetch(url)
        res = await res.json()
        if(res.code == 203 ){
            e.reply('呵，不想回答',true)
