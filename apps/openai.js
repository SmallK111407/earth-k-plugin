...
export class example extends plugin {
    ...
    async ly(e) {
        ...
        if(e.msg.includes('小原角色')){
            sj = 0
           js = e.msg.replace(/小原角色/, "").trim()
            e.reply('角色切换至'+js)
            return
        }
        if(sj == 1){
        ...
        let kw = e.msg.replace(/小原/, "").trim()
        let url = 'https://api.lolimi.cn/api/ai/ya?key=JMHz65uMHMZ3WCpl4jQ1U1blRr&length=1.1&ren='+js+'&msg='+kw
